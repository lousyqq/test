...
/*
            //建立資料庫連線
            SqlConnection conn = new SqlConnection("Data Source=Sariel;Initial Catalog=Teach;User ID=testuser;Password=test");
            SqlCommand cmd = null;

            string sSql = "SELECT TOP (1) * FROM [Teach].[dbo].[Announcement]";
            conn.Open();
